...
uses
  ...
  ,UGestionCursistasHospedados, UPuestoDeTrabajo
  ,UDataModuleControlDelSistema
  ,UPantalla, XiButton, LbButton,
...
begin
  ...
  if MessageDlg('¿Desea cancelar la validación en el sistema HostalWare?' + #13 + 'La aplicación se cerrará.', mtConfirmation, [mbYes, mbNo], 0) = mrYes
    then
      begin
    ...
    then
      begin
          ...
          if (   (lbledtUsuario.Text<> '') and (lbledtContrasenna.Text<> '')   )
            then
              begin
                ...
                try
                HostalWare.EstablecerConexionConLaBD(ContrasennaBD, UsuarioBD, NombreBaseDeDatos, NombreServer, Intranet);
                except
                on Exception do
                ...
                if (HostalWare.ExisteConexionConLaBD = True)
                  then
                    begin
                      if (HostalWare.ExisteElPuestoDeTrabajoConUsuario(usuario))
                        then
                          begin
                            //Aquí es donde debo verificar si existe alguien loggeado en el sistema HCCAP
                            if (HostalWare.HayAlguienLoggeado = False)
                              then
                                begin
                                  HostalWare.BuscarPuestoDeTrabajo(usuario);

                                  //Verifico si la contraseña coincide
                                  if (DBTextContrasenna.Field.Text = HostalWare.EncriptarSeudoaleatorio(contrasenna))
                                    then
                                      begin
                                        //Busco al usuario como si fuese Administrador, para luego comprobar si lo halló
                                        HostalWare.SelecAdministrador(usuario);
                                        ...
                                        HostalWare.LoggearAlUsuario(usuario, Privilegio, NombreUsuario, PrimerApellido, SegundoApellido);
                                    ...
                                    begin // Hay alguien supuestamente loggeado. Debo verificar si aun lo está (posible apagón)
                                      ...
                                      frmValidacionPuestoDeTrabajo.Caption:= 'HostalWare. Verificando conexión';
                        ...
                        begin
                          ShowMessage('No existe conexión en estos momentos con la base de datos.' + #13 + 'Es imposible realizar la validación en el sistema HostalWare.' + #13 + 'El sistema se cerrará.');
                          Application.Terminate;
                        end;
              end
                else
          ...
          begin
            ...
            //Saco un cartel
            Application.MessageBox('Falta el nombre del Server. Sin él no se puede conectar el Sistema HostalWare a la base de datos.','Error',mb_Ok + mb_IconError);
...
var 
  hMenuHandle : HMENU;
begin
  frmValidacionPuestoDeTrabajo.Caption:= 'HostalWare. Validación de Entrada';
  ...
  if (HostalWare.ExisteConexionConLaBD)
    then
      begin
        ...
        HostalWare.ActualizarPermanencia;
      end;
