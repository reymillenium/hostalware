...
uses
  ...
  UPuestoDeTrabajo, UGestionCursistasHospedados, LbButton,
    ...
    then
      begin
        if (edtContrasenna.Text = edtRepetirContrasenna.Text)
          then
            begin
              if (HostalWare.ExisteElPuestoDeTrabajoConUsuario(edtUsuario.Text) = False)
                then
                  begin
                    ...
                    HostalWare.InsertarPuestoDeTrabajo(TempPuestoDeTrabajo);
                    ...
                    //Inserto la Traza
                    HostalWare.GuardarTrazaDelSistema('Creó la Cuenta de Usuario ' + '"' + Usuario + '"' + ' con privilegios de ' + Privilegios + ' y perteneciente a ' + NombreCompleto + '.');
...
begin
  ...
  TempPuestoDeTrabajo.Contrasenna:= HostalWare.EncriptarSeudoaleatorio(frase);
