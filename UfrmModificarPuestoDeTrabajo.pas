...
uses
  ...
  UPuestoDeTrabajo, UGestionCursistasHospedados, UDataModuleControlDelSistema,
  LbButton,
        ...
        if (   (DBedtNombre.Text<>'') and (DBedtPrimerApellido.Text<>'') and (DBedtSegundoApellido.Text<>'') and (DBedtUsuario.Text<>'') and (edtViejaContrasenna.Text<>'') and (edtNuevaContrasenna.Text<>'')  )
          then
            begin //Están todos los componentes llenos; con habilitación de contraseña
              ...
              if ( HostalWare.DesencriptarSeudoaleatorio(edtViejaContrasenna.Text) = DBTextContrasenna.Field.Text)
                then
                    ...
                    if (edtNuevaContrasenna.Text = edtRepetirContrasenna.Text)
                      then
                        begin //Las nuevas contraseñas coinciden, contraseña vieja coincide; componentes llenos; con habilitación de contraseña
                          ...
                          if (   (HostalWare.ExisteElPuestoDeTrabajoConUsuario(DBedtUsuario.Text) = False) or (DBedtUsuario.Text = TempViejoPuestoDeTrabajo.ID_Usuario)   )
                            then
                              begin
                                ...
                                if (HostalWare.ElUsuarioEstaLoggeado(DBTextID_Usuario.Field.Text) = True)
                                  then
                                    begin //** Es el mismo que está loggeado
                                      ...
                                      if (   (ViejoPrivilegios = 'Administrador') and (NuevoPrivilegios = 'Operador')   )
                                        then
                                          begin //Se los redujo
                                            if MessageDlg('¿Desea reducirse a si mismo los privilegios dentro del sistema HostalWare? Si lo hace será expulsado(a) automáticamente de la pestaña de <<Control del Sistema>>.', mtConfirmation, [mbYes, mbNo], 0) = mrYes
                                              then
                                                begin //Escoge reducirlos de todas formas
                                                  //Llamo al procedimiento que modifica la Cuenta de Usuario
                                                  HostalWare.ModificarPuestoDeTrabajo(TempViejoUsuario, TempNuevoPuestoDeTrabajo);

                                                  //Actualizo el DBGrid de los Cuentas de Usuario
                                                  HostalWare.MostrarPuestosDeTrabajo;

                                                  //Actualizo el Loggin
                                                  HostalWare.ModificarLoggin(ViejoUsuario, NuevoUsuario, NuevoPrivilegios, TempNuevoPuestoDeTrabajo.Nombre, TempNuevoPuestoDeTrabajo.PrimerApellido, TempNuevoPuestoDeTrabajo.SegundoApellido);

                                                  //Inserto la Traza
                                                  HostalWare.GuardarTrazaDelSistema('Modificó la Cuenta de Usuario ' + '"' + ViejoUsuario + '"' + ' con privilegios de ' + ViejoPrivilegios + ' y perteneciente a ' + ViejoNombreCompleto + ', por el de ' + '"' + NuevoUsuario + '"' + ' con privilegios de ' + NuevoPrivilegios + ' y perteneciente a ' + NuevoNombreCompleto +  ' y además cambió la contraseña. Como estando loggeado modificó su propia Cuenta de Usuario, las trazas a continuación se generarán con estos nuevos datos. Se redujo a si mismo sus privilegios, por lo que fue expulsado de la pestaña de Control del Sistema.');
                                        ...
                                        begin //** No es el mismo que está loggeado
                                          ...
                                          //Inserto la Traza
                                          HostalWare.GuardarTrazaDelSistema('Modificó la Cuenta de Usuario ' + '"' + ViejoUsuario + '"' + ' con privilegios de ' + ViejoPrivilegios + ' y perteneciente a ' + ViejoNombreCompleto + ', por el de ' + '"' + NuevoUsuario + '"' + ' con privilegios de ' + NuevoPrivilegios + ' y perteneciente a ' + NuevoNombreCompleto +  ' y además cambió la contraseña.');
                            ...
                            begin //** Es el mismo que está loggeado
                              ...
                              if (   (ViejoPrivilegios = 'Administrador') and (NuevoPrivilegios = 'Operador')   )
                                ...
                                begin //** No es el mismo que está loggeado
                                  ...
                                  //Inserto la Traza
                                  HostalWare.GuardarTrazaDelSistema('Modificó el Cuenta de Usuario ' + '"' + ViejoUsuario + '"' + ' con privilegios de ' + ViejoPrivilegios + ' y perteneciente a ' + ViejoNombreCompleto + ', por el de ' + '"' + NuevoUsuario + '"' + ' con privilegios de ' + NuevoPrivilegios + ' y perteneciente a ' + NuevoNombreCompleto +  '.');
...
begin
  ...
  if (CheckBoxCambiarContrasenna.Checked = True)
    then
      aPuestoDeTrabajo.Contrasenna:= HostalWare.EncriptarSeudoaleatorio(edtNuevaContrasenna.Text)
        else
      ...
      if (Index= 0)
        then
          Canvas.Brush.Color:= clPaleGreen  //Operadores
              ...
              begin
                if (   (Index= 1) and (DBedtUsuario.Text<> 'hostalware')   )
                  then
