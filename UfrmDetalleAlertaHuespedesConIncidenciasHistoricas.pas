...
begin
  ...
  if (HostalWare.ExisteConexionConLaBD = True)
    then
      begin
        //Busco los huéspedes con antecedentes de incidencias en la BD
        HostalWare.MostrarHuespedesConIncidenciasSoloHistoricas;

        //Muestro la cantidad de huéspedes con antecedentes de incidencias, en dependencia de la cantidad de ellas
        if (HostalWare.CantidadHuespedesConIncidenciasSoloHistoricas = 1)
          then
            begin
              lblCantidadHuespedesConIncidencias.Caption:= 'TOTAL = 1 huésped con antecedentes de incidencias';
            end
              else
                begin
                  lblCantidadHuespedesConIncidencias.Caption:= 'TOTAL = ' + IntToStr(HostalWare.CantidadHuespedesConIncidenciasSoloHistoricas) + ' huéspedes con antecedentes de incidencias';
                end;
      end
        else
          begin
